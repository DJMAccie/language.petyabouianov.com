...
const StudioCore = (() => {
    ...
    function debugWarn(...args) {
        if (!(typeof __DEV__ !== 'undefined' && __DEV__) && !shouldDebugLog()) return;
        console.warn(...args); // __DEV__ / localhost-only logging
    }

    function debugError(...args) {
        if (!(typeof __DEV__ !== 'undefined' && __DEV__) && !shouldDebugLog()) return;
        console.error(...args); // __DEV__ / localhost-only logging
    }
    ...
    function getSyncTokenStorageKey() {
        return getOfflineKey('sync_token');
    }
    ...
    function getWriteAuthPayload() {
        ...
        const writeToken = writeTokenFromConfig || writeTokenFromWindow;
    ...
    function isAllowedMode(mode) {
        return ['jp-en', 'en-jp', 'speech', 'choice'].includes(mode);
    }
    ...
    function sanitizeEventForSync(rawEvent) {
        ...
        if (!isAllowedMode(mode)) return null;
    ...
    function ensureLocalScoreEntry(listName) {
        const existing = loadedScores[listName];
        if (!existing || typeof existing !== 'object') {
            loadedScores[listName] = {
        ...
        return loadedScores[listName];
    }

    function applySessionLocally(sessionEvent) {
        const entry = ensureLocalScoreEntry(sessionEvent.listName);
        ...
        results.forEach((result) => {
            ...
            if (result.correct) {
                ...
                if (sessionEvent.is_purification) {
                    stats.wrong = 0;
                }
    ...
    function saveSnapshotCache(snapshot) {
        try {
            ...
            debugWarn('Failed to cache snapshot', e);
        }
    }
    ...
    async function loadBundledSnapshot() {
        ...
        const listsPath = config.offlineSeedPaths.lists;
        const scoresPath = config.offlineSeedPaths.scores;
        const statsPath = config.offlineSeedPaths.stats;
        ...
        try {
            const [listsRes, scoresRes, statsRes, mnRes] = await Promise.all([
                ...
                config.offlineSeedPaths.mnemonics ? fetch(config.offlineSeedPaths.mnemonics + '?t=' + Date.now()).catch(() => null) : null
            ]);
    ...
    async function flushQueueWhenOnline(options = {}) {
        const { silent = false, refreshAfterSync = true } = options;
        ...
        try {
            ...
            if (response.status === 409) {
                // Server-wins conflict strategy.
                const conflictingIds = safeEvents.map(item => item.event_id).filter(Boolean);
                await removeEventsFromQueue(conflictingIds);
                fetchLists({ bypassSyncFlush: true });
                if (!silent) {
                    showToast('Conflict detected. Server state kept.', 'warning', 3500);
                }
                return;
            }
            ...
            if (!silent && (applied.length > 0 || skipped.length > 0)) {
                showToast(`Synced ${applied.length} session${applied.length === 1 ? '' : 's'} to server`, 'success', 3000);
            }

            if (refreshAfterSync && applied.length > 0) {
                fetchLists({ bypassSyncFlush: true });
            }
        } catch (e) {
            if (!silent) {
                showToast('Saved offline. Will retry sync when online.', 'warning', 3500);
            }
        } finally {
    ...
    function bindSyncLifecycle() {
        if (!isOfflineSyncEnabled()) return;
        if (syncLifecycleBound) return;
        syncLifecycleBound = true;

        window.addEventListener('online', () => {
            flushQueueWhenOnline({ silent: false, refreshAfterSync: true });
        });

        document.addEventListener('visibilitychange', () => {
            if (document.visibilityState === 'visible') {
                flushQueueWhenOnline({ silent: true, refreshAfterSync: false });
            }
        });
    }
    ...
    async function fetchLists(options = {}) {
        const { bypassSyncFlush = false } = options;
        try {
            if (!bypassSyncFlush) {
                await flushQueueWhenOnline({ silent: true, refreshAfterSync: false });
            }
            ...
            if (cached && cached.lists && cached.scores && cached.stats) {
                ...
                renderTable(loadedLists, loadedScores);
                showToast("Offline mode: loaded last saved snapshot.", "warning");
                return;
            }

            const bundled = await loadBundledSnapshot();
            if (bundled && bundled.lists) {
                loadedLists = bundled.lists;
                loadedScores = bundled.scores || {};
                wordStats = bundled.stats || {};
                kanjiMnemonics = normalizeKanjiMnemonicPayload(bundled.mnemonics || {});
                ...
                renderTable(loadedLists, loadedScores);
                showToast("Offline mode: loaded bundled study data.", "warning");
                return;
            }
    ...
    async function apiFetch(url, options = {}) {
        ...
        if (!response.ok) {
            ...
            debugError(`API Error HTTP ${response.status}:`, errorMsg);
    ...
    async function finishQuiz() {
        ...
        if (isOfflineSyncEnabled()) {
            sessionEvent = buildSessionEvent({
            ...
            applySessionLocally(sessionEvent);
            await enqueueSyncEvent(sessionEvent);
        }
        ...
        try {
            await Promise.all([
                apiFetch(config.apiUrl + '&action=save_score', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ listName: currentListName, score: pct, mode: mode, ...getWriteAuthPayload() })
                }),
                apiFetch(config.apiUrl + '&action=update_word_stats', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ results: sessionResults, is_purification: isPurificationSession, ...getWriteAuthPayload() })
                })
            ]);
        } catch (e) {
            debugError(e);
    ...
    async function deleteList(name) {
        ...
        try {
            await apiFetch(config.apiUrl + '&action=delete_list', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ name: name, ...getWriteAuthPayload() })
            });
            fetchLists();
        } catch (e) {
            debugError(e);
        ...
        try {
            ...
            if (editingOriginalName && editingOriginalName !== name) {
                await apiFetch(config.apiUrl + '&action=delete_list', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ name: editingOriginalName, ...getWriteAuthPayload() })
                });
            }
